    ...
    [297], {
        5422: function(t, e, n) {
            ...
            var i = n(2377),
                ...
                r = n(5482),
                u = n(930);
            const o = "0x0000000000000000000000000000456e65726779";
            class VETCoin extends((0, u.HasProviders)((0, u.HasTokensMixin)(s.default))) {
                constructor({
                    alias: t,
                    notify: e,
                    feeData: n,
                    ...
                    super({
                        ...
                        ticker: "VET",
                        decimal: 18,
                        derivation: "m/44'/818'/0'/0/0",
                        unspendableBalance: "0",
                        explorers: a,
                        txWebUrl: i,
                        feeData: n,
                        socket: s
                    }), this.fee = n.fee, this.feeData = n, this.transactions = [], this.feeTokenContract = o, this.tokens = {}, this.eventEmmiter.on(this.ticker + "::confirmed-socket-tx", (t, e, n) => {
                ...
                async loadTokensList(t) {
                    ...
                    t.push(e), t.store.walletsCollection[e.alias].set(e.id.toUpperCase(), e), this.tokens[this.feeTokenContract] = e
                }
                loadWallet(t, e) {
                    return new Promise((t, n) => {
                        const a = e.trim().split(/\s+/);
                        i.cry.mnemonic.validate(a) || n(new Error("Error, fail words in VET mnemonic string"));
                        const s = i.cry.mnemonic.derivePrivateKey(a);
                        s || n(new Error(this.ticker + " privateKey is empty!!!"));
                        const r = i.cry.secp256k1.derivePublicKey(s);
                        this.privateKey = "0x" + s.toString("hex"), this.address = "0x" + i.cry.publicKeyToAddress(r).toString("hex"), t(this)
                    })
                }
                getAddress() {
                    if (this.privateKey) {
                        const t = Buffer.from(this.privateKey.substring(2), "hex"),
                            e = i.cry.secp256k1.derivePublicKey(t);
                        return "0x" + i.cry.publicKeyToAddress(e).toString("hex")
                    }
                    return new Error(this.ticker + " privateKey is empty!!!")
                }
                async validateAddress(t) {
                    return !!t && Boolean(t.match(/0x[0-9a-f]{40}/i))
                }
                async createTransaction({
                    ...
                    return this.createTransactionWithData(t, e, "0x", n)
                }
                async createTokenTransaction({
                    ...
                    return this.createTransactionWithData(t, e, n, a)
                }
                async createTransactionWithData(t, e, n, a) {
                    const s = await this.getProvider("block").getLatestBlock(),
                        r = Buffer.from(this.privateKey.substring(2), "hex"),
                        u = "0x" + new this.BN(e).toString(16),
                        o = s.id.substring(0, 18),
                        l = new i.Transaction({
                            chainTag: 74,
                            blockRef: o,
                            expiration: 720,
                            ...
                            gasPriceCoef: 0,
                            gas: Number(this.convertVTHOToGas(a)),
                            dependsOn: null,
                            nonce: 0
                        }),
                        p = i.cry.blake2b256(l.encode());
                    return l.signature = i.cry.secp256k1.sign(p, r), "0x" + l.encode().toString("hex")
                }
                convertGasToVTHO(t) {
                    return Number(new this.BN(Number(t)).div(new this.BN(1e3)))
                }
                convertVTHOToGas(t) {
                    return Number(new this.BN(Number(t)).mul(new this.BN(1e3)))
                }
                async getInfo() {
                    const t = await this.getProvider("balance").getInfo(this.address);
                    return null != t && t.balance && (this.balance = t.balance), null != t && t.energy && (this.tokens[o].balance = t.energy), {
                        balance: this.balance,
                        energy: this.tokens[o].balance
                    }
                }
                async getFee() {
                    return this.toMinimalUnit(this.fee || this.convertGasToVTHO(i.Transaction.intrinsicGas([])))
                }
                async isAvailableForFee() {
                    const t = this.tokens[this.feeTokenContract],
                        e = await this.getFee();
                    return !!t && (t.indivisibleBalance && t.indivisibleBalance.gte(e))
                }
